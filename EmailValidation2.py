#https://www.youtube.com/watch?v=8ihJd5s3bh8
    ...
    if len(user_email) > 7:
        if re.match("^.+@(\[?)[a-zA-Z0-9-.]+.([a-zA-Z]{2,3}|[0-9]{1,3})(]?)$" , user_email) !=None:
            return True
                ...
                messagebox.showinfo('Information','Email should contain at least one small,capital,special character and number')
                return False
...
def validateAllFields():
        if v_fName.get() == "":
            ...
            if (status):
                Insert_Data()
                messagebox.showinfo('Information' , 'User Register Completed succsessfully')
        else:
            
            messagebox.showinfo('Information' , 'User Register Not Completed succsessfully')
...
#heading
lb_heading=Label(window,text="Registration" , width=20, font=("bold" , 20),bg="light blue")
...
Radiobutton(window,text="Male" , bg="pink", padx=5,variable=v_gender,value=1).place(x=250,y=330)
Radiobutton(window,text="Female" , bg="pink", padx=20,variable=v_gender,value=2).place(x=300,y=330)
...
droplist.config(width=16, bg="pink")
...
btn_register= Button(window,text="Register",command=validateAllFields,bg="green",fg="white",font=("bold",10)).place(x=150,y=450)
btn_clear= Button(window,text="Clear",command=clearAllFields,bg="green",fg="white",font=("bold",10)).place(x=250,y=450)
btn_existinguser= Button(window,text="Existing User?",command=callNewScreen,bg="green",fg="white",font=("bold",10)).place(x=330,y=450)

#=================DB CONNect============================================================

def Insert_Data():
    print("****Hello Yash*****")
    connection=pymysql.connect(host="localhost",user="root",password="", database="classprofile")
    cursor=connection.cursor()
    print("****Connection OPened*****")
    InsertQuery=" Insert into Student (Stud_name,Pass,Email,Gender,Country,Skills,Phone) Values ("
    InsertQuery=InsertQuery+" '"+v_fName.get()+"' , '"+v_pwd.get()+"','"+v_emailId.get()+"', '"+str(v_gender.get())+"', '"+v_country.get()+"', '"+v_skills.get()+"','"+str(v_phoneNo.get())+"')"
    print(InsertQuery)
